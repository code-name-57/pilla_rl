...
def get_cfgs():
    ...
    reward_cfg = {
        ...
        "reward_scales": {
            # Remove locomotion rewards - focus on recovery
            "tracking_lin_vel": 0.0,
            "tracking_ang_vel": 0.0,
            
            # Basic penalties
            "lin_vel_z": -1.0,              # Light penalty for vertical movement
            "base_height": -2.0,            # Moderate penalty for wrong height
            "action_rate": -0.02,           # Penalize rapid actions slightly
            "similar_to_default": -0.1,     # Very light penalty for joint deviation
            
            # Main recovery rewards
            "upright_orientation": 20.0,    # Strong reward for upright posture
            "recovery_progress": 30.0,      # Main reward for recovery progress
            "minimize_base_roll": 15.0,     # Focus on roll recovery (key for upside-down)
            
            # Supporting rewards
            "stability": 5.0,               # Reward controlled movement
            "legs_not_in_air": 8.0,         # Encourage grounded, controlled leg movement
            "energy_efficiency": 3.0,       # Encourage efficient movements
            "forward_progress": 2.0,        # Small reward for forward motion when upright
        },
    }
    command_cfg = {
        "num_commands": 3,
