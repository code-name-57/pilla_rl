...
class Go2Env:
    ...
    def _reward_upright_orientation(self):
        ...
        # Use a combination approach for better gradients
        roll_score = torch.cos(roll_rad)
        pitch_score = torch.cos(pitch_rad)
        
        # Combined upright score - multiplicative so both roll and pitch need to be good
        upright_score = roll_score * pitch_score
        
        # Convert to positive reward (1 when upright, -1 when upside-down)
        # But reduce the negative impact when upside-down to focus on positive progress
        return torch.clamp(upright_score, -0.5, 1.0)
    ...
    def _reward_recovery_progress(self):
        ...
        # Height component - but don't let it dominate until orientation is reasonable
        height_error = torch.abs(self.base_pos[:, 2] - self.reward_cfg["base_height_target"])
        height_factor = torch.exp(-height_error / 0.15)
        
        # Weight the height component based on orientation progress
        # When orientation is poor, focus on orientation. When good, height becomes important
        height_weight = torch.clamp(orientation_progress * 2.0 - 0.5, 0.0, 1.0)  # Starts mattering when orientation > 25%
        
        return orientation_progress * (0.8 + 0.2 * height_weight * height_factor)
    ...
    def _reward_standup_height(self):
        # Strong reward for achieving proper standing height once upright
        roll_rad = torch.abs(self.base_euler[:, 0] * torch.pi / 180)
        pitch_rad = torch.abs(self.base_euler[:, 1] * torch.pi / 180)
        
        # Only provide height reward when robot is reasonably upright
        is_upright = (roll_rad < torch.pi / 6) & (pitch_rad < torch.pi / 6)  # Within 30 degrees
        
        # Height reward - exponential reward for being close to target height
        height_error = torch.abs(self.base_pos[:, 2] - self.reward_cfg["base_height_target"])
        height_reward = torch.exp(-height_error / 0.1)  # Sharp reward curve for precise height
        
        # Only give height reward when upright, zero otherwise
        return torch.where(is_upright, height_reward, torch.zeros_like(height_reward))
    
    def _reward_complete_standup(self):
        # Bonus reward for achieving both upright orientation AND proper height
        roll_rad = torch.abs(self.base_euler[:, 0] * torch.pi / 180)
        pitch_rad = torch.abs(self.base_euler[:, 1] * torch.pi / 180)
        height_error = torch.abs(self.base_pos[:, 2] - self.reward_cfg["base_height_target"])
        
        # Check if robot is fully stood up
        is_orientation_good = (roll_rad < torch.pi / 12) & (pitch_rad < torch.pi / 12)  # Within 15 degrees
        is_height_good = height_error < 0.05  # Within 5cm of target height
        is_stable = torch.sum(torch.square(self.base_ang_vel), dim=1) < 2.0  # Not spinning wildly
        
        is_fully_standup = is_orientation_good & is_height_good & is_stable
        
        return torch.where(is_fully_standup, torch.ones_like(roll_rad), torch.zeros_like(roll_rad))
    
    def _reward_height_when_upright(self):
        # Progressive height reward that increases as orientation improves
        roll_rad = torch.abs(self.base_euler[:, 0] * torch.pi / 180)
        pitch_rad = torch.abs(self.base_euler[:, 1] * torch.pi / 180)
        
        # Orientation quality (0 to 1, where 1 is perfectly upright)
        roll_quality = torch.exp(-roll_rad / (torch.pi / 6))  # Decay over 30 degrees
        pitch_quality = torch.exp(-pitch_rad / (torch.pi / 6))  # Decay over 30 degrees
        orientation_quality = roll_quality * pitch_quality
        
        # Height reward (0 to 1, where 1 is perfect height)
        height_error = torch.abs(self.base_pos[:, 2] - self.reward_cfg["base_height_target"])
        height_quality = torch.exp(-height_error / 0.08)  # Sharp reward for correct height
        
        # Combined reward: height importance scales with how upright the robot is
        return orientation_quality * height_quality
