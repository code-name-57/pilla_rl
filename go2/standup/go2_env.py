...
class Go2Env:
    ...
    def reset_idx(self, envs_idx):
        ...
        # Randomize base position (small variations around initial position)
        self.base_pos[envs_idx] = self.base_init_pos + gs_rand_float(
            lower=torch.tensor([-0.2, -0.2, 0.1], device=gs.device),
            upper=torch.tensor([0.2, 0.2, 0.5], device=gs.device),
            shape=(len(envs_idx), 3),
            device=gs.device,
        )
        
        # Randomize base orientation (roll, pitch, yaw)
        random_euler = gs_rand_float(
            lower=torch.tensor([-180, -180, -180], device=gs.device) * torch.pi / 180,
            upper=torch.tensor([180, 180, 180], device=gs.device) * torch.pi / 180,
            shape=(len(envs_idx), 3),
            device=gs.device,
        )
